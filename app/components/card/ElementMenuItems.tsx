...
export function ContextMenuItems({ elementId, elementType, onCopy, onBringToFront, onBringForward, onSendBackward, onSendToBack, onDelete }: MenuItemsProps) {
  return (
    <div className="bg-[#1a2332] border border-gray-700 rounded-lg shadow-lg p-1 min-w-[180px]">
      <ContextMenu.Item
        className="text-sm px-3 py-2 outline-none cursor-pointer hover:bg-[#26C4E1]/10 rounded flex items-center gap-2 text-gray-100"
        onSelect={() => onCopy(elementId, elementType)}
      >
        <Copy className="w-4 h-4 text-[#26C4E1]" /> Copy
      </ContextMenu.Item>
      ...
      <ContextMenu.Item
        className="text-sm px-3 py-2 outline-none cursor-pointer hover:bg-[#a855f7]/10 rounded flex items-center gap-2 text-gray-100"
        onSelect={() => onBringToFront(elementId, elementType)}
      >
        <ArrowUpToLine className="w-4 h-4 text-[#a855f7]" /> Bring to Front
      </ContextMenu.Item>
